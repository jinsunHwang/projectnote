...
router.get("/list", function(req, res, next) {
  ...
  var sql =
    ...
    "  b.*, " +
    "  (select filename from note_info_image where info_seq = a.info_seq) as image_filename " +
    "from note_keep as a left join note_info as b " +
    " on (a.info_seq = b.seq) " +
    "where a.member_seq = ? " +
...
router.post("/:member_seq/:info_seq", function(req, res, next) {
  ...
  var sql_select =
    "select count(*) as cnt from note_keep where member_seq = ? and info_seq = ?;";
  var sql_insert = "insert into note_keep (member_seq, info_seq) values(?, ?);";
  var sql_update = "update note_info set keep_cnt = keep_cnt+1 where seq = ? ";
...
router.delete("/:member_seq/:info_seq", function(req, res, next) {
  ...
  var sql_delete =
    "delete from note_keep where member_seq = ? and info_seq = ? ";
  var sql_update = "update note_info set keep_cnt = keep_cnt-1 where seq = ? ";
